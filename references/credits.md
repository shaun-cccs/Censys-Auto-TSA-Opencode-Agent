...
guess a credit figure - measure it with `utils/censys_credits.py`, or quote the
published rate below.** `utils/censys_tsa.py` reports the credits a run consumed
...
| Credit balance / usage endpoints (`censys_credits.py`) | **0** | yes, control delta 0 |
| `utils/cve_lookup.py` (cve.org / NVD, not Censys) | **0** | n/a |
| `utils/tsa_report.py` (renders locally, never calls Censys) | **0** | n/a |
...
python utils/censys_credits.py usage --start-date 2026-07-01 --granularity daily
python utils/censys_credits.py usage --granularity monthly --by-consumer
python utils/censys_credits.py -f json usage --by-consumer   # -f is a global flag, before the subcommand
```
...
TSA. Attribute spend with `--by-consumer`, and treat a `censys_tsa.py` credit
